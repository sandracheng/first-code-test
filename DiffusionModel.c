...
this model is 1D. The code is for a particle-wide section of sphere. We drip many particles onto the same spot (continuous dripping), and see the final positions of all of them.

the sphere has symmetry about it, so we can use this code for one particle-wide section to extrapolate for the rest of the sphere 
...
int DiffusionModel() {  
  ...
  For now, this model uses prob=0.5 of moving forward and backward, but these numbers should actually change as the particle moves further into the acrylic
  ...
  */
  ...
  int pCount;
	  ...
	  for (int pCount = 0; pCount<totParticle; pCount++)  {     // loop through total number of particles,Yan...
                ...
                for (int step = 0; step < 1000; step++) {         // loop through steps.  Generate random number b/w 0 and 100 to determine how the particle moves at each step 
